...
We decided to use 3 different models for the prediction of fast growing firms : Logit, Lasso and Random Forest.
...
```
We started by carrying out a probability prediction by logit. Used 5-fold cross-validation to estimate models.
...
grid.arrange(summary_results, ncol = 1)
...
# Average Growth among invested companies
data_holdout[data_holdout$holdout_prediction == "Hyp.Growth",] %>% group_by(HyperGrowth_f) %>% 
  summarize(AvgAnnGrowth = mean(CAGR),Firms_Selected = n()) 
