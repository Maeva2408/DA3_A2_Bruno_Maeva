...
Our 5 Logistic regression models gradually incorporate more & more of the variable described groups above, where our final model, similarly to LASSO, incorporates all available variables. Finally, our Random Forest model considers all variable groups except interactions.  
We used a 5-fold cross-validation to estimate models and then we selected the best model based on expected loss.
...
We observed that the lowest RMSE is 36.1 % and it is given by models X3, Lasso and Random forest. However the other models had RMSE around 36% as well, the difference is very small, for example model X4 has a RMSE of 36,2%

AUC

The highest AUC is given by the random forest with a AUC of 65,1%. We are 15% better than random choosing and 7% better than the base model X1. However, like for the RMSE, the model X4 is very close to the Random forest with a AUC of 64,8%


EXPECTED LOSS

We selected the best threshold for each model by minimizing expected loss.
The smaller expected loss is 0.793 is the model X4. Considering that it is a very theoretical concept, we wont interpret it.


We decided to choose the Model X4 as best model. Firstly, the expected loss was the lowest and it was the criteria we chose as most important. Secondly, the AUC and RMSE were very close to the best one of the models.
